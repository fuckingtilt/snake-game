...
class Snake {
    constructor() {
        ...
        this.path = [];
        this.length = 10; // Початкова довжина менша
        this.baseRadius = 10;
    ...
    grow() { this.length += 3; } // Змійка росте повільніше
}
...
class Food {
    ...
    spawn() {
        ...
        this.color = this.isCoin ? var(--coin-neon) : var(--danger-neon);
        this.shadow = this.isCoin ? var(--coin-glow) : var(--danger-glow);
    ...
    draw() {
        ...
        if (this.isCoin) {
            ...
            ctx.font = `bold ${this.radius}px var(--game-font)`;
...
function startGame() {
    ...
    food = new Food();
    score = 0;
    velocity = { x: 0, y: -1 }; // Починаємо рухатись вгору
    running = true;
    updateUI();
...
function loadData() {
    ...
    currentSkin = skins.find(s => s.id === skinId);
}
        ...
        item.onclick = () => {
            if (isPurchased) {
                ...
                if (coins >= skin.price) {
                    ...
                    // Можна додати анімацію "недостатньо коштів"
                }
            }
        };
...
document.addEventListener('touchmove', e => {
    ...
    // Оновлюємо початкову точку, щоб свайп був плавнішим
