...
// Поки тестуєш на комп'ютері, залиш як є.
...
// --- Ігровий цикл та логіка (без змін) ---
...
function endGame() {
    ...
    if (score > highscore) {
        highscore = score;
        localStorage.setItem('snake_highscore_v8', highscore);
    }
    
    elements.finalScore.textContent = score;
    saveDataToServer(); // Відправляємо фінальний результат на сервер
    switchScreen('gameOverScreen');
}

// --- НОВІ ФУНКЦІЇ ДЛЯ РОБОТИ З СЕРВЕРОМ ---
async function loadDataFromServer() {
    try {
        ...
        console.log("Лідерборди завантажено з сервера.");
    } catch (error) {
        console.error("Не вдалося завантажити лідерборди:", error);
        leaderboardScores = [];
        leaderboardCoins = [];
    }
}
...
async function saveDataToServer() {
    try {
        ...
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        console.log("Результат відправлено на сервер.");
        // Показуємо повідомлення про новий рекорд, якщо він був
        const data = await response.json();
        if (data.newRecord) {
             elements.newRecordText.classList.remove('hidden');
        }
    } catch (error) {
        console.error("Не вдалося відправити результат:", error);
    }
}

async function showLeaderboards() {
    await loadDataFromServer();
    populateLeaderboard(elements.highScoresList, leaderboardScores, 'score');
    populateLeaderboard(elements.coinScoresList, leaderboardCoins, 'coins');
    switchScreen('leaderboardScreen');
}


// --- ОНОВЛЕНІ ФУНКЦІЇ ЗБЕРЕЖЕННЯ/ЗАВАНТАЖЕННЯ ---
...
function loadData() {
    ...
    highscore = parseInt(localStorage.getItem('snake_highscore_v8')) || 0;
    coins = parseInt(localStorage.getItem('snake_coins_v8')) || 0;
    purchasedSkins = JSON.parse(localStorage.getItem('snake_purchasedSkins_v8')) || [0];
    const skinId = parseInt(localStorage.getItem('snake_currentSkin_v8')) || 0;
    currentSkin = skins.find(s => s.id === skinId) || skins[0];
}

function saveData() {
    // Зберігаємо тільки локальні дані гравця, які не стосуються лідербордів
    localStorage.setItem('snake_highscore_v8', highscore);
    localStorage.setItem('snake_coins_v8', coins);
    localStorage.setItem('snake_purchasedSkins_v8', JSON.stringify(purchasedSkins));
    localStorage.setItem('snake_currentSkin_v8', currentSkin.id);
}
