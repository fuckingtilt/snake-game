...
// Це гарантує, що всі кнопки та елементи будуть на місці і гра не зависне на завантаженні.
document.addEventListener('DOMContentLoaded', () => {

    // --- ГЛОБАЛЬНІ КОНСТАНТИ ТА НАЛАШТУВАННЯ ГРИ ---
    // Всі налаштування винесені сюди для легкості редагування
    const VERSION = '11_ENGINE'; // Змінюємо версію для скидання старих збережень
    const COLORS = { 
    ...
    // Цей об'єкт буде керувати всіма аспектами гри.
    const GameEngine = {
        state: 'LOADING', 
        ...
        init() {
            ...
            // Імітуємо невелику затримку, щоб гравець побачив екран завантаження
            setTimeout(() => this.switchState('MENU'), 500);
        },

        // --- Керування станом гри (меню, гра, кінець гри і т.д.) ---
        switchState(newState) {
            if (this.state === newState) return; // Не перемикаємо, якщо стан той самий
            this.state = newState;
            
            // Керуємо видимістю екранів
            ...
            // Запускаємо/зупиняємо ігровий цикл залежно від стану
            if (newState === 'PLAYING' || newState === 'BOSS_FIGHT') this.startGameLoop();
            else this.stopGameLoop();

            // Виконуємо дії при вході в певний стан
        ...
        // --- Ігровий цикл ---
        gameLoop() {
            this.update(); // Оновлення логіки
            this.draw();   // Малювання
            this.animationFrameId = requestAnimationFrame(() => this.gameLoop());
        },
        startGameLoop() { this.animationFrameId = requestAnimationFrame(() => this.gameLoop()); },
        stopGameLoop() { if (this.animationFrameId) cancelAnimationFrame(this.animationFrameId); this.animationFrameId = null; },

        // --- Оновлення логіки гри (один кадр) ---
        update() {
            this.updatePowerups();
            ...
            this.entities.foods.forEach((foodItem, index) => {
                if (Math.hypot(this.entities.snake.head.x - foodItem.x, this.entities.snake.head.y - foodItem.y) < this.entities.snake.baseRadius + foodItem.radius) {
                    ...
                    if (foodItem.isCoin) { this.player.coins++; } 
                        ...
                        this.gameplay.combo++; 
                        this.gameplay.comboTimer = GAME_SETTINGS.MAX_COMBO_TIME; 
                        this.gameplay.score += this.gameplay.combo; 
                        ...
                        if (this.gameplay.score > 0 && this.gameplay.score >= GAME_SETTINGS.BOSS_TRIGGER_SCORE && (this.gameplay.score - this.gameplay.combo) < GAME_SETTINGS.BOSS_TRIGGER_SCORE) {
                            this.entities.boss = new Boss();
        ...
        checkBossCollisions() {
            this.entities.boss.weakPoints.forEach((point, index) => {
                if(Math.hypot(this.entities.snake.head.x - point.x, this.entities.snake.head.y - point.y) < this.entities.snake.baseRadius + point.radius) {
                    this.entities.boss.takeDamage();
                    this.createParticleBurst(point.x, point.y, COLORS.BOSS_NEON);
                    this.entities.boss.weakPoints.splice(index, 1);
            ...
            if(this.entities.boss.isDefeated()) {
                ...
                this.updateUI();
                this.switchState('PLAYING');
                this.entities.foods.push(new Food());
                elements['boss-health-container'].classList.add('hidden');
            }
            const isOutOfBounds = this.entities.snake.head.x < 0 || this.entities.snake.head.x > elements.canvas.width || this.entities.snake.head.y < 0 || this.entities.snake.head.y > elements.canvas.height;
            if ((isOutOfBounds && !this.gameplay.activePowerups.ghost) || this.entities.snake.checkCollisionSelf()) this.switchState('GAME_OVER');
        ...
        // --- Управління даними гравця (локально) ---
        loadLocalData() { 
            ...
            try {
                ...
                this.leaderboards.scores = JSON.parse(localStorage.getItem(`snake_leaderboardScores_${VERSION}`)) || [];
                this.leaderboards.coins = JSON.parse(localStorage.getItem(`snake_leaderboardCoins_${VERSION}`)) || [];
            } catch (e) { console.error("Помилка завантаження даних:", e); this.player.purchasedSkins = [0]; }
        },
        saveLocalData() { 
            try {
                ...
                localStorage.setItem(`snake_leaderboardScores_${VERSION}`, JSON.stringify(this.leaderboards.scores));
                localStorage.setItem(`snake_leaderboardCoins_${VERSION}`, JSON.stringify(this.leaderboards.coins));
            } catch (e) { console.error("Помилка збереження даних:", e); }
        },
        ...
        updateIndicatorsUI() { 
            elements['combo-bar'].style.width = `${(this.gameplay.comboTimer / GAME_SETTINGS.MAX_COMBO_TIME) * 100}%`; 
            elements['combo-text'].textContent = `x${this.gameplay.combo}`;
            elements['powerup-timers'].innerHTML = ''; 
            for(const type in this.gameplay.activePowerups) { 
                const timeLeft = Math.ceil((this.gameplay.activePowerups[type] - Date.now()) / 1000); 
        ...
        renderShop() { 
            ...
            skins.forEach(skin => { 
                ...
                item.addEventListener('click', () => { 
                    if (isPurchased) { 
                        this.player.currentSkin = skin; this.saveLocalData(); this.renderShop(); 
                        if (this.entities.snake) this.entities.snake.applySkin(this.player.currentSkin);
                    } else if (this.player.coins >= skin.price) { 
                        this.player.coins -= skin.price; this.player.purchasedSkins.push(skin.id); this.player.currentSkin = skin; 
                        this.saveLocalData(); this.renderShop(); this.updateUI(); 
                    } 
                }); 
        ...
        populateLeaderboard(listElement, data, type) { 
            listElement.innerHTML = ''; if (data.length === 0) { listElement.innerHTML = '<li>Тут поки пусто...</li>'; return; } 
            data.forEach((entry, index) => { 
                const li = document.createElement('li'); const value = type === 'score' ? entry.score : entry.coins; 
                li.innerHTML = `<span class="rank">${index + 1}.</span><span class="name">${entry.name}</span><span class="score">${value}</span>`; 
                if (entry.name === this.player.id) li.classList.add('current-player'); listElement.appendChild(li); 
            }); 
        },
        updateLeaderboards() { 
            const currentBest = this.leaderboards.scores.find(e => e.name === this.player.id)?.score || 0; 
            if (this.gameplay.score > 0 && this.gameplay.score > currentBest) { 
                this.addOrUpdateLeaderboard(this.leaderboards.scores, { name: this.player.id, score: this.gameplay.score }, 'score'); 
                elements.newRecordText.classList.remove('hidden'); 
            } 
            this.addOrUpdateLeaderboard(this.leaderboards.coins, { name: this.player.id, coins: this.player.coins }, 'coins');
        },
        ...
        getPlayerId() {
            try { const tg = window.Telegram.WebApp; const user = tg.initDataUnsafe.user; if (user) return user.username ? `@${user.username}` : user.first_name || 'PLAYER'; } catch (e) {}
