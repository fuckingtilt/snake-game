...
class Snake {
    constructor() {
        ...
        this.path = [];
        this.length = 10;
        this.baseRadius = 6; // Змійка стала меншою
        this.segmentDistance = 3;
    ...
    update(direction) {
        ...
        if (this.path.length > this.length * this.segmentDistance) {
            this.path.shift();
        }
    }
    draw() {
    ...
    checkCollisionSelf() {
        for (let i = 0; i < this.path.length - Math.floor(this.length * this.segmentDistance / 2); i++) {
            if (Math.hypot(this.head.x - this.path[i].x, this.head.y - this.path[i].y) < this.baseRadius / 2) {
                return true;
            }
        }
        return false;
...
class Boss {
    constructor() {
        this.x = elements.canvas.width / 2;
        this.y = 100;
        this.radius = 30;
        this.health = 10;
        this.maxHealth = 10;
        this.vx = 2;
        this.vy = 1;
        this.color = COLORS.BOSS_NEON;
    }
    update() {
        this.x += this.vx;
        this.y += this.vy;
        if (this.x < this.radius || this.x > elements.canvas.width - this.radius) this.vx *= -1;
        if (this.y < this.radius || this.y > elements.canvas.height / 2) this.vy *= -1;
    }
    draw() {
        ctx.fillStyle = this.color;
        ctx.beginPath();
    ...
    takeDamage() {
        this.health--;
        elements['boss-health-bar'].style.width = `${(this.health / this.maxHealth) * 100}%`;
    }
}
...
function checkCollisions() {
    for (let i = foods.length - 1; i >= 0; i--) {
        ...
        if (Math.hypot(snake.head.x - foodItem.x, snake.head.y - foodItem.y) < snake.baseRadius + foodItem.radius) {
            if (foodItem.isCoin) {
                coins++;
                gameData.quests.coinsCollected++;
            } else {
                score++;
                snake.grow();
                gameData.quests.foodEaten++;
            }
            foods.splice(i, 1); // Видаляємо з'їдену їжу
        }
    }
    ...
    if (snake.head.x < 0 || snake.head.x > elements.canvas.width || snake.head.y < 0 || snake.head.y > elements.canvas.height || snake.checkCollisionSelf()) {
        endGame();
    }
}
...
function startGame() {
    ...
    foods = [new Food()];
    score = 0;
    gameData.quests.scoreGainedThisRun = 0;
    gameData.lastBossScore = 0;
    velocity = { x: 0, y: -1 };
    running = true;
    updateUI();
...
function startBossFight() {
    ...
    boss = new Boss();
    foods = [];
    elements['boss-ui'].classList.remove('hidden');
    elements.canvas.classList.add('boss-mode');
}
...
function endBossFight() {
    ...
    boss = null;
    foods.push(new Food());
    elements['boss-ui'].classList.add('hidden');
    elements.canvas.classList.remove('boss-mode');
}
...
function updateUI() {
    ['score', 'highscore', 'coins', 'shopCoins'].forEach(id => {
        if (elements[id]) elements[id].textContent = window[id.replace('shop', '').toLowerCase()];
    });
}
...
function getPlayerId() {
    try {
        ...
        let id = localStorage.getItem('snake_playerId_v11');
        if (!id) {
            id = `PLAYER-${Math.random().toString(16).substr(2, 4).toUpperCase()}`;
            localStorage.setItem('snake_playerId_v11', id);
        }
        return id;
    }
}
...
function loadData() {
    ...
    const savedData = JSON.parse(localStorage.getItem('snake_gameData_v11'));
    const today = new Date().toDateString();

    gameData = {
        lastLogin: new Date(0).toDateString(),
        quests: { foodEaten: 0, coinsCollected: 0, scoreGainedThisRun: 0, claimed: [] },
        ...savedData
    };
    
    if (new Date(gameData.lastLogin).toDateString() !== today) {
        gameData.quests = { foodEaten: 0, coinsCollected: 0, scoreGainedThisRun: 0, claimed: [] };
    }

    highscore = parseInt(localStorage.getItem('snake_highscore_v11')) || 0;
    coins = parseInt(localStorage.getItem('snake_coins_v11')) || 0;
    purchasedSkins = JSON.parse(localStorage.getItem('snake_purchasedSkins_v11')) || [0];
    leaderboardScores = JSON.parse(localStorage.getItem('snake_leaderboard_v11')) || [];
    const skinId = parseInt(localStorage.getItem('snake_currentSkin_v11')) || 0;
    currentSkin = skins.find(s => s.id === skinId) || skins[0];
}

function saveData() {
    localStorage.setItem('snake_gameData_v11', JSON.stringify(gameData));
    localStorage.setItem('snake_highscore_v11', highscore);
    localStorage.setItem('snake_coins_v11', coins);
    localStorage.setItem('snake_purchasedSkins_v11', JSON.stringify(purchasedSkins));
    localStorage.setItem('snake_currentSkin_v11', currentSkin.id);
    localStorage.setItem('snake_leaderboard_v11', JSON.stringify(leaderboardScores));
}
...
function populateQuests() {
    ...
    QUESTS.forEach(q => {
        ...
        item.innerHTML = `
            ...
            <button class="claim-button" data-quest-id="${q.id}" ${progress < q.goal || isClaimed ? 'disabled' : ''}>${isClaimed ? '✓' : `+${q.reward}`}</button>
        `;
...
function populateShop() {
    ...
    skins.forEach(skin => {
        ...
        if (!isPurchased && skin.price > 0) item.classList.add('locked');
        
        item.innerHTML = `
            <div class="skin-preview" style="background: linear-gradient(90deg, ${skin.colors[0]}, ${skin.colors[1]})"></div>
            <div>${skin.name}</div>
            ${isPurchased ? (isEquipped ? '<div class="equipped-badge">✓</div>' : '') : `<div class="skin-price"><span class="coin-icon"></span> ${skin.price}</div>`}
        `;
...
function showLeaderboards() {
    populateLeaderboard(elements.highScoresList, leaderboardScores);
    switchScreen('leaderboardScreen');
}

function populateLeaderboard(listElement, data) {
    ...
    data.forEach((entry, index) => {
        ...
        if (entry.name === playerId) li.classList.add('current-player');
...
function addScoreToLeaderboard(newEntry) {
    ...
    if (existingIndex !== -1) {
        if (newEntry.score > leaderboardScores[existingIndex].score) {
            leaderboardScores[existingIndex].score = newEntry.score;
        }
    } else {
        leaderboardScores.push(newEntry);
    }
    leaderboardScores.sort((a, b) => b.score - a.score);
    if (leaderboardScores.length > LEADERBOARD_SIZE) {
