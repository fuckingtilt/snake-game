...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const GameEngine = {
        ...
        switchState(newState) {
            ...
            const screenMap = { 'MENU': 'mainMenu', 'PLAYING': 'gameScreen', 'BOSS_FIGHT': 'gameScreen', 'GAME_OVER': 'gameOverScreen', 'SHOP': 'shopScreen', 'LEADERBOARD': 'leaderboardScreen' };
        ...
        startGameLoop() { if (!this.running) { this.running = true; this.gameLoop(); } },
        stopGameLoop() { this.running = false; if (this.animationFrameId) cancelAnimationFrame(this.animationFrameId); },
        ...
        updatePowerups() { 
            if (this.gameplay.comboTimer > 0) this.gameplay.comboTimer--; else this.gameplay.combo = 1; 
            for(const type in this.gameplay.activePowerups) if(Date.now() > this.gameplay.activePowerups[type]) delete this.gameplay.activePowerups[type]; 
            ...
            this.entities.foods.forEach((foodItem, index) => {
                if (Math.hypot(this.entities.snake.head.x - foodItem.x, this.entities.snake.head.y - foodItem.y) < this.entities.snake.baseRadius + foodItem.radius) {
                    if (foodItem.isCoin) { this.player.coins++; } 
                        ...
                        if (this.gameplay.score > 0 && this.gameplay.score % GAME_SETTINGS.BOSS_TRIGGER_SCORE < this.gameplay.combo) {
        ...
        populateLeaderboard(listElement, data, type) { 
            ...
            data.forEach((entry, index) => { 
                ...
                if (entry.name === this.player.id) li.classList.add('current-player'); 
        ...
        getPlayerId() {
            ...
            if (!id) {
                ...
                localStorage.setItem(`snake_playerId_${VERSION}`, id);
            }
            return id;
        }
    };
