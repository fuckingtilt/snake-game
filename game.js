...
const skins = [
    ...
    { id: 16, name: 'Ангельський', price: 1700, colors: ['#ffffff', '#fffacd'] },
    { id: 17, name: 'Демонічний', price: 1800, colors: ['#8b0000', '#ff0000'] },
...
class Food { constructor() { this.spawn(); } spawn() { this.isCoin = false; this.radius = 8; this.color = COLORS.DANGER_NEON; this.x = Math.random() * (elements.canvas.width - 40) + 20; this.y = Math.random() * (elements.canvas.height - 40) + 20; } draw() { ctx.beginPath(); ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2); ctx.fillStyle = this.color; ctx.shadowColor = this.color; ctx.shadowBlur = 20; ctx.fill(); ctx.shadowBlur = 0; if (this.isCoin) { ctx.fillStyle = '#a07d00'; ctx.font = `bold ${this.radius+2}px "${getComputedStyle(document.documentElement).getPropertyValue('--game-font').trim()}"`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('$', this.x, this.y + 1); } } }
...
function gameLoop() {
    ...
    foods.forEach(f => f.draw()); if(powerup) powerup.draw();
    snake.draw();
...
function checkCollisions() {
    foods.forEach((foodItem, index) => {
        if (Math.hypot(snake.head.x - foodItem.x, snake.head.y - foodItem.y) < snake.baseRadius + foodItem.radius) {
            if (foodItem.isCoin) { coins++; updateCoinLeaderboard(); } else { combo++; comboTimer = MAX_COMBO_TIME; score += combo; snake.grow(); if(Math.random() < POWERUP_SPAWN_CHANCE && !powerup) powerup = new PowerUp(); }
...
function getPlayerId() { try { const tg = window.Telegram.WebApp; return tg.initDataUnsafe.user.username ? `@${tg.initDataUnsafe.user.username}` : (tg.initDataUnsafe.user.first_name || 'PLAYER'); } catch (e) { let id = localStorage.getItem('snake_playerId_v7'); if (!id) { id = `PLAYER-${Math.random().toString(16).substr(2, 4).toUpperCase()}`; localStorage.setItem('snake_playerId_v7', id); } return id; } }
function loadData() { playerId = getPlayerId(); highscore = parseInt(localStorage.getItem('snake_highscore_v7')) || 0; coins = parseInt(localStorage.getItem('snake_coins_v7')) || 0; purchasedSkins = JSON.parse(localStorage.getItem('snake_purchasedSkins_v7')) || [0]; const skinId = parseInt(localStorage.getItem('snake_currentSkin_v7')) || 0; currentSkin = skins.find(s => s.id === skinId) || skins[0]; leaderboardScores = JSON.parse(localStorage.getItem('snake_leaderboardScores_v7')) || []; leaderboardCoins = JSON.parse(localStorage.getItem('snake_leaderboardCoins_v7')) || []; }
function saveData() { localStorage.setItem('snake_highscore_v7', highscore); localStorage.setItem('snake_coins_v7', coins); localStorage.setItem('snake_purchasedSkins_v7', JSON.stringify(purchasedSkins)); localStorage.setItem('snake_currentSkin_v7', currentSkin.id); localStorage.setItem('snake_leaderboardScores_v7', JSON.stringify(leaderboardScores)); localStorage.setItem('snake_leaderboardCoins_v7', JSON.stringify(leaderboardCoins)); }
...
function addOrUpdateLeaderboard(leaderboard, newEntry, key) { const existingIndex = leaderboard.findIndex(e => e.name === newEntry.name); if (existingIndex !== -1) { if(newEntry[key] > leaderboard[existingIndex][key]) leaderboard[existingIndex][key] = newEntry[key]; } else if (leaderboard.length < LEADERBOARD_SIZE || newEntry[key] > (leaderboard[leaderboard.length - 1]?.[key] || 0)) { leaderboard.push(newEntry); } leaderboard.sort((a, b) => b[key] - a[key]); if (leaderboard.length > LEADERBOARD_SIZE) leaderboard.pop(); }

// --- Обробники подій ---
document.addEventListener('click', e => { if (e.target.matches('.menu-button, .skin-item, .tab-button')) { if(navigator.vibrate) navigator.vibrate(50); }});
